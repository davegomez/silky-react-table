...
test('sort descending based on first item of each element by default', () => {
  const options = merge(defaults, { sortDescending: true })
  expect(sortData(data, options)).toEqual([
    [3, 'bar', 5],
    [2, 'qux', 4],
    [1, 'foo', 6],
...
test('string ascending sort based on second item of each element', () => {
  const options = merge(defaults, { sortByColumn: 1 })
  expect(sortData(data, options)).toEqual([
    [3, 'bar', 5],
    [1, 'foo', 6],
    [2, 'qux', 4],
...
test('string descending sort based on second item of each element', () => {
  const options = merge(defaults, { sortByColumn: 1, sortDescending: true })
  expect(sortData(data, options)).toEqual([
    [2, 'qux', 4],
    [1, 'foo', 6],
    [3, 'bar', 5],
...
test('number ascending sort based on last item of each element', () => {
  const options = merge(defaults, { sortByColumn: 2 })
  expect(sortData(data, options)).toEqual([
    [2, 'qux', 4],
    [3, 'bar', 5],
    [1, 'foo', 6],
...
test('number descending sort based on last item of each element', () => {
  const options = merge(defaults, { sortByColumn: 2, sortDescending: true })
  expect(sortData(data, options)).toEqual([
    [1, 'foo', 6],
    [3, 'bar', 5],
    [2, 'qux', 4],
